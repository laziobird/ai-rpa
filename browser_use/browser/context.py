...
class BrowserContext:
	...
	async def _click_element_node(self, element_node: DOMElementNode) -> Optional[str]:
		...
		##Todo日志
		##logger_p.debug(
		##	" Click action 1/2 , page url : " + page.url.__str__())
		try:
	...
	async def get_element_by_index(self, index: int) -> ElementHandle | None:
		...
		logger.debug("ME-16-缓存里面拿元素 | get_element_by_index : ", element_handle)
		return element_handle

	async def get_dom_element_by_index(self, index: int) -> DOMElementNode:
		selector_map = await self.get_selector_map()
		logger.debug("ME-15-缓存里面拿元素 | get_dom_element_by_index |  Selector map:", selector_map)
